...

{
    ...
    public function getMonthlySalesForYear($year)
    {
        ...
        foreach (range(1, 12) as $month) {
            ...
            $monthlySales[] = [
                'month' => Carbon::createFromFormat('m', $month)->locale('fr')->isoFormat('MMMM'), // Mois en français
                'totalSales' => $sales,
    ...
    // vente de la semaine
    public function getDailySalesThisWeek() 
    {
        Carbon::setLocale('fr');
        ...
        foreach (range(0, 6) as $dayOffset) {
            ...
            $salesForDay = Sale::whereDate('created_at', $currentDay)
                            ->get();
        
            $totalSalesForDay = $salesForDay->sum(function ($sale) {
                return $sale->salePayed;
            });
            
            $dailySales[] = [
                'day' => $currentDay->locale('fr')->isoFormat('dddd'), // Jour en français
                'date' => $currentDay->format('Y-m-d'),
                'total_sales' => $totalSalesForDay
            ];
        }
